---
title: "Agent 的骨架：一文讲透 Agent Runtime"
source: "https://mp.weixin.qq.com/s/5sftvwAZL4Hj5v40HZ1rVw"
author:
  - "[[二曲线工程师]]"
published:
created: 2026-06-30
description: "Agent 工程系列第 4 篇： Agent Runtime"
tags:
  - "clippings"
---
二曲线工程师 工程师的第二曲线 *2026年6月5日 21:50*
...
如果你把这三篇的内容拼在一起，会发现一件事：Loop 要转、Context 要组装、工具要调用、结果要处理——这些事情不会自己发生。背后有一个东西在协调，它就是 Agent Runtime。

---

*本文是「工程师的第二曲线」Agent 工程系列第 **04** 篇。*
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/xmGIAYiaJQ284ia9G3lC7HHicAofvDo0HZOib28Blia8Kz6JZ5GVBZGa6QNwlQD9A4uDT5kMq5lpjxyWver1IUpFIHd0OUM7Xo1ko8f6pmPibZ9vo/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=0)
...
**上下文组装** 每轮调用 LLM 之前，Runtime 负责把 System Prompt、执行历史、工具定义、当前状态拼成完整的 Context 塞给模型。这是 Context Engineering 的执行层。

**状态管理（State）** 当前跑到第几轮、已经收集了哪些信息、任务进展如何——这些运行时状态由 Runtime 维护，不是 LLM 的事。LLM 每次调用都是无状态的，状态要靠外部系统保存。

**终止判断** Agent Loop 什么时候停？达到最大轮数、模型返回最终答案、不再请求工具调用、工具连续失败——Runtime 负责执行这些终止逻辑。没有明确的终止条件，Loop 就可能无限跑下去。

**风险控制** 哪些操作需要人工审批？哪些工具有调用频率限制？哪些参数需要脱敏处理？Runtime 在 LLM 和工具执行之间做一层拦截，防止 Agent 做出不该做的事。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/xmGIAYiaJQ2ibRZApnHpjica0D5jaAZLhAjlib7JiavjlfhU4iaWpUqr38FkjjHvgmfTib7wlcSOektLqWO9FNb05jGDIXrolaQVjkruVBm39CjH2s/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=1)
...
```
1while True:
2    action = llm.call(context)
3    result = tool.run(action)
4    context.append(result)
```
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/xmGIAYiaJQ2icL8k54B0q5FAFA9vV30iaXkrsgFMAbgEoq43evQdROxQwsteW418EVOBkTcWYrOAdsIRicEsJrDoZqT3WgiblfQsqJfbGHLNrr4Y/640?wx_fmt=png&from=appmsg&watermark=1#imgIndex=2)
...
#Agent开发、 #大模型应用、 #AI工程师、#后端开发、 `#AgentRuntime`
